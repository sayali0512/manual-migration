...
For installing the infrastructure for Moodle navigate to the azure portal.(i.e portal.azure.com) 

In the home section go the resource group section and add a new a resource group for the Moodle infrastructure click here 
...
Tags are name-value pairs that are used to organize resources in Azure Portal Click here 
...
Standard Load balancer: An Azure load balancer is a Layer-4 (TCP, UDP) load balancer that provides high availability by distributing incoming traffic among healthy VMs. A load balancer health probe monitors a given port on each VM and only distributes traffic to an operational VM. Click here 
...
After giving the mandatory values click on review and create. Click here 
...
An Azure Database for MySQL server is created with a defined set of compute and storage resources. Click here 
